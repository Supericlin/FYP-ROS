...
const char* Application::MQTT_TAG = "MQTT_Client";
...
void Application::CheckNewVersion() {
  ...
  while (true) {
    ...
    //                 std::string message =
    //                 std::string(Lang::Strings::NEW_VERSION) +
    ...
    //                     char buffer[64];
    //                     snprintf(buffer, sizeof(buffer), "%d%% %zuKB/s",
    //                     progress, speed / 1024);
    //                     display->SetChatMessage("system", buffer);
    //                 });

    //                 // If upgrade success, the device will reboot and never
    //                 reach here
...
void Application::ToggleChatState() {
  ...
  if (device_state_ == kDeviceStateIdle) {
    Schedule([this]() {
      ...
      SetListeningMode(realtime_chat_enabled_ ? kListeningModeRealtime
...
void Application::Start() {
  ...
  if (realtime_chat_enabled_) {
    ESP_LOGI(TAG,
             "Realtime chat enabled, setting opus encoder complexity to 0");
  ...
  protocol_->OnIncomingJson([this, display](const cJSON* root) {
    ...
    } else if (strcmp(type->valuestring, "goal") == 0) {
        auto status = cJSON_GetObjectItem(root, "status");
        if (status != NULL && strcmp(status->valuestring, "goalReach") == 0) {
        ESP_LOGI(TAG, "Goal reached, triggering TTS.");
        Schedule([this, display]() {
        display->SetChatMessage("assistant", "Navigate to destination success.");
        // 如有 TTS 播放模組，這裡可調用相應接口進行語音播報
        });
      }
  ...
  // Initialize a simple MQTT client for notifications
  ESP_LOGI(MQTT_TAG, "Initializing MQTT client for notifications");
  esp_mqtt_client_config_t mqtt_cfg = {};
  // Use the broker config properly with the new structure
  mqtt_cfg.broker.address.uri = "mqtt://192.168.0.224";  // Use hardcoded value for now
  // Set client ID
  char client_id[64];
  // Use c_str() to convert std::string to const char*
  snprintf(client_id, sizeof(client_id), "esp32_%s", board.GetUuid().c_str());
  mqtt_cfg.credentials.client_id = client_id;

  mqtt_client_ = esp_mqtt_client_init(&mqtt_cfg);
  
  mqtt_client_ = esp_mqtt_client_init(&mqtt_cfg);
  esp_mqtt_client_register_event(mqtt_client_, static_cast<esp_mqtt_event_id_t>(ESP_EVENT_ANY_ID), mqtt_event_handler, this);
...
void Application::MainLoop() {
  while (true) {
    auto bits = xEventGroupWaitBits(event_group_, SCHEDULE_EVENT, pdTRUE,
...
void Application::ReadAudio(std::vector<int16_t>& data, int sample_rate,
                            int samples) {
  auto codec = Board::GetInstance().GetAudioCodec();
...
// MQTT event handler
void Application::mqtt_event_handler(void* handler_args, esp_event_base_t base, int32_t event_id, void* event_data) {
    Application* app = static_cast<Application*>(handler_args);
    esp_mqtt_event_handle_t event = static_cast<esp_mqtt_event_handle_t>(event_data);
    
    switch (event_id) {
        case MQTT_EVENT_CONNECTED:
            ESP_LOGI(app->MQTT_TAG, "MQTT client connected");
            // Subscribe to the status topic
            esp_mqtt_client_subscribe(app->mqtt_client_, "robot/status", 1);
            esp_mqtt_client_subscribe(app->mqtt_client_, "robot/command", 1);
            break;
            
        case MQTT_EVENT_DISCONNECTED:
            ESP_LOGI(app->MQTT_TAG, "MQTT client disconnected, will auto-reconnect");
            break;
            
        case MQTT_EVENT_DATA:
            app->on_mqtt_data(event);
            break;
...
void Application::on_mqtt_data(esp_mqtt_event_handle_t event) {
    // Null-terminate the strings for safe handling
    char* topic = strndup(event->topic, event->topic_len);
    char* data = strndup(event->data, event->data_len);
    
    if (topic && data) {
        ESP_LOGI(MQTT_TAG, "Received message on topic %s: %s", topic, data);
        
        // Check for destination reached messages
        if (strcmp(topic, "robot/status") == 0 && strstr(data, "goalReached")) {
            ESP_LOGI(MQTT_TAG, "Goal reached notification detected!");
            
            // Schedule destination reached processing on main thread
            Schedule([this]() {
                OnDestinationReached();
            });
        }
        // Check for wake up commands
        else if (strcmp(topic, "robot/command") == 0 && strstr(data, "wakeup:destination_reached")) {
            ESP_LOGI(MQTT_TAG, "Destination reached wakeup command received");
...
// Add the destination reached handling method
void Application::OnDestinationReached() {
    ESP_LOGI(TAG, "Destination reached notification received");
    ...
    auto display = Board::GetInstance().GetDisplay();
    display->ShowNotification("Destination reached");
    
    // Only wake up the device if it's in idle state
    if (device_state_ == kDeviceStateIdle) {
        ESP_LOGI(TAG, "Waking up device for destination reached notification");
        
        // Wake up the device and connect to the server
        SetDeviceState(kDeviceStateConnecting);
        ...
        // Set the listening mode based on your preference
        SetListeningMode(realtime_chat_enabled_ ? kListeningModeRealtime 
                                                : kListeningModeAutoStop);
        
        // Use the existing protocol for sending custom events if possible
        cJSON* event = cJSON_CreateObject();
        cJSON_AddStringToObject(event, "type", "custom_event");
        cJSON_AddStringToObject(event, "event", "destination_reached");
